...
function selectionTextForCodex(items) {
  return items
    ...
    .map((item, index) => {
      const source = item.source?.url ? ui(`
来源：{0}`, item.source.url) : "";
      const link = item.type === "link" && item.src ? ui(`
链接：{0}`, item.src) : "";
      const page = item.type === "page" && item.src
        ? ui(`
页面：{0}{1}`, item.src, item.pageContent ? ui(`
页面正文（{0}）：
{1}`, item.pageContentMode || ui("已渲染"), item.pageContent) : "")
        : "";
      const taskOutput = item.type === "task" ? cardTextOutput(item).trim() : "";
      const result = taskOutput ? ui(`
最新回答：{0}`, taskOutput.slice(0, 40_000)) : "";
      const terminal = item.type === "terminal"
        ? ui(`
命令：{0}
控制台输出：{1}`, item.text || "", String(item.terminalOutput || "").slice(-20_000))
        : "";
      const local = ["file", "folder"].includes(item.type) ? ui(`
内容：
{0}`, cardTextOutput(item).slice(0, 40_000)) : "";
      const document = ["document", "code"].includes(item.type) ? ui(`
内容：
{0}`, String(item.text || "").slice(0, 40_000)) : "";
      return ui(`[{0}] {1}卡片
{2}{3}{4}{5}{6}{7}{8}{9}`, index + 1, itemTypeLabel(item), itemLabel(item), result, terminal, local, document, link, page, source);
    })
    .join("\n\n");
}
...
function selectionContextForCodex(items) {
  ...
  const imageMetadata = images
    .map((item, index) => ui("[图片 {0}] {1}{2}", index + 1, item.alt || item.source?.title || ui("未命名图片"), item.source?.url ? ui(`
来源：{0}`, item.source.url) : ""))
    .join("\n\n");
  return [
    ui("圈选内容共 {0} 项：{1} 个文字、{2} 个文档、{3} 个代码、{4} 个本地文件、{5} 张图片、{6} 个链接、{7} 个页面、{8} 个视频卡片、{9} 个任务、{10} 个控制台。", items.length, items.filter(item => item.type === "text").length, items.filter(item => item.type === "document").length, items.filter(item => item.type === "code").length, items.filter(item => ["file", "folder"].includes(item.type)).length, images.length, items.filter(item => item.type === "link").length, items.filter(item => item.type === "page").length, items.filter(item => item.type === "video").length, items.filter(item => item.type === "task").length, items.filter(item => item.type === "terminal").length),
    textContent ? ui(`文字、链接、页面与控制台：

{0}`, textContent) : "",
    imageMetadata ? ui(`图片名称与来源：
...
async function imageDataForCodex(item) {
  ...
  if (!response.ok) throw new Error(ui("图片读取失败：HTTP {0}", response.status));
  const blob = await response.blob();
  if (!/^image\/(png|jpe?g|webp)$/i.test(blob.type)) {
    throw new Error(ui("“{0}”不是 Codex 支持的 PNG、JPEG 或 WebP 图片", itemLabel(item)));
  }
  if (blob.size > MAX_CODEX_IMAGE_BYTES) throw new Error(ui("“{0}”超过 10 MB", itemLabel(item)));
  return readFileAsDataUrl(blob);
}

async function videoDataForCodex(item) {
  const response = await fetch(item.src);
  if (!response.ok) throw new Error(ui("视频读取失败：HTTP {0}", response.status));
  const blob = await response.blob();
  if (blob.type !== "video/mp4") throw new Error(ui("当前只支持 MP4 视频后期"));
  if (blob.size > MAX_CODEX_VIDEO_BYTES) throw new Error(ui("视频超过 48 MB，无法添加口播"));
  return readFileAsDataUrl(blob);
}
...
async function runWhiteboardCodex(mode, taskItem = null, contextOptions = {}) {
  ...
  const generationContext = taskItem
    ...
    : { scope: "sources", label: ui("圈选内容"), messages: [], messageIds: [], messageId: "" };
  ...
  if (!selection.length && generationContext.scope === "sources") {
    if (taskItem) {
      taskItem.taskStatus = "error";
      taskItem.taskError = ui("原始素材已不存在，请重新圈选，或改为基于当前回答");
  ...
  if (taskItem) {
    ...
    taskItem.taskProgress = selection.some(item => item.type === "page")
      ? ui("正在读取页面内容")
      : ui("正在理解{0}", generationContext.label);
  ...
  try {
    ...
    if (taskItem) {
      ...
      taskItem.taskStatus = "error";
      taskItem.taskError = error?.message || ui("页面内容读取失败");
      taskItem.taskProgress = "";
      updateTaskItemElement(taskItem);
      scheduleSave();
      updateCodexChatControls();
    } else {
      setStatus(error?.message || ui("页面内容读取失败"), true);
    }
    return;
  }
  if (taskItem) {
  ...
  if (aiRuntime === "claude" && ["image", "image-gen", "video", "video-post"].includes(mode)) {
    const error = ui("Claude Code 当前支持文字总结、知识卡和任务对话；生图和视频请切换到 Codex 或 AGY");
    if (taskItem) {
      preparingTaskItemIds.delete(taskItem.id);
      taskItem.taskStatus = "error";
      taskItem.taskError = error;
  ...
  if (imageItems.length > MAX_CODEX_IMAGES) {
    const error = ui("单次最多分析 {0} 张图片，请减少圈选内容", MAX_CODEX_IMAGES);
    if (taskItem) {
      preparingTaskItemIds.delete(taskItem.id);
      taskItem.taskStatus = "error";
      taskItem.taskError = error;
  ...
  const shortcutLabels = {
    text: ui("总结{0}", generationContext.label),
    image: ui("基于{0}生成手稿信息图", generationContext.label),
    "image-gen": ui("基于{0}进行 AI 自由绘图", generationContext.label),
    video: ui("基于{0}生成视频", generationContext.label),
    "video-post": ui("为视频添加口播与字幕")
  };
...
补充要求：{1}`, shortcutLabels[mode], additionalInstruction)
          : shortcutLabels[mode],
        kind: "conversation"
      })
    : null;
  ...
  if (taskItem) {
    ...
    taskItem.text = "";
    taskItem.taskProgress = ui("正在理解{0}", generationContext.label);
  ...
  updateWhiteboardCodexStatus(task, ui("正在理解{0}", generationContext.label));
  updateSelectionUi();
  updateCodexChatControls();
  if (!taskItem) setStatus(ui("正在理解素材"));
  ...
  try {
    ...
    const shortcutPrompts = {
      text: ui("请把指定输入作为一个整体理解。输出文字总结：先给出一句话结论，再列出关键观点、重要信息、内容之间的关系、冲突点和可执行事项。不要遗漏输入信息，也不要臆测。"),
      knowledge: ui("请把指定输入提炼成一张结论优先、可长期复用的业务知识卡，只输出 Markdown。若输入是任务卡，以“最新回答”为主体，忽略任务标题、状态和执行回执。严格结构：# 标题（不超过16字）；紧接一个引用块 > 一句话结论（不超过60字）；## 核心要点（3到5条，每条只写一个有依据的事实或判断，不超过50字）；仅在输入确有依据时增加 ## 业务规则、## 关键指标、## 风险与待确认，每节1到3条；最后写 ## 来源，最多5条，只列可确认的卡片名、文件名或 URL。除来源 URL 外，总字数不超过 500 个中文字符。省略没有可靠信息的章节，禁止重复、空话、背景铺垫和“未识别”占位章节，不得补造事实、规则、指标或来源。若输入含文件夹，只能把目录名和文件名视为索引，不能推断文件正文，并在风险中用一句话说明边界。"),
      image: ui(`请把指定输入作为一个整体理解，并为一张重点突出、吸引人眼球的手稿信息图制定内容方案。成图采用粗粝纸张纹理与明显的手写字感，只呈现内容、结构和关系，可使用便签、手绘箭头、圈画、马克笔高亮和少量涂鸦修饰，但装饰不能压过信息。只输出合法 JSON，不要 Markdown 或代码块。格式必须是：{"title":"不超过24字","summary":"不超过120字","highlights":["3到6个按逻辑顺序排列的核心内容节点，每条不超过40字，第一条是最重要结论"],"relations":[{"from":"起点概念，不超过16字","to":"终点概念，不超过16字","label":"关系说明，不超过12字"}],"imageNotes":["按输入图片顺序给出每张图不超过24字的说明"],"theme":"ocean、violet、warm、forest 四选一"}。relations 提取 2 到 5 条最关键的因果、层级、流程或依赖关系。内容层级必须清楚，重点突出，文字精炼。禁止加入产品或品牌眉标、AI 生成说明、用户原始输入、内容摘录、白板名称、来源、时间、生成方式、页脚或水印。提炼内容关系与结论，不要臆测。`),
      "image-gen": ui("请把指定输入作为一个整体理解，使用 Codex 内置 image-gen 直接创作一张完整图片。允许根据内容自由构图，但视觉语言必须与模板做图一致：明亮温暖的米白或暖奶油纸面、明显但克制的纸纤维与手绘笔触、深蓝或深棕墨线、珊瑚/蓝/橙少量重点色、清晰居中的文字区块，以及便签、手绘箭头、圈画和马克笔高亮。画面要重点突出、吸引人，并准确呈现核心内容、结构和关系；中文要有自然手写感且清楚易读，装饰不能压过信息。禁止暗黑科技风、黑色或深色大底、霓虹光效、赛博朋克、玻璃拟态、金属质感、强烈渐变和阴郁低对比画面。不要照搬固定模板排版，不要加入无用眉标、来源脚注、水印或生成说明，也不要臆造事实。"),
      video: ui("请把指定输入作为一个整体理解，提炼一条清晰叙事主线，并制作成解释型视频。只使用本次明确提供的内容作为事实依据，不要回到未选择的旧话题，也不要臆造事实。"),
      "video-post": ui("使用输入的口播文案，为指定视频添加本地 Kokoro 配音与同步字幕。")
    };
    const prompt = additionalInstruction
      ? ui(`{0}

用户补充要求：{1}`, shortcutPrompts[mode], additionalInstruction)
      : shortcutPrompts[mode];
    const sourceVideo = mode === "video-post" ? selection.find(item => item.type === "video" && item.src) : null;
    if (mode === "video-post" && !sourceVideo) throw new Error(ui("原视频已不存在，请重新选择视频"));
    ...
    if (taskItem) {
      ...
      taskItem.taskStatus = "error";
      taskItem.taskError = error?.message || ui("AI 任务启动失败");
      taskItem.taskProgress = "";
      taskItem.taskRunId = "";
      taskItem.taskCompletedAt = Date.now();
      appendTaskEvent(taskItem, { stage: "failed", label: ui("任务启动失败"), detail: taskItem.taskError, status: "error" });
      const promptElement = itemElement(taskItem.id)?.querySelector(".task-prompt");
      if (promptElement) promptElement.value = additionalInstruction;
      updateTaskItemElement(taskItem);
      scheduleSave();
    }
    if (!taskItem) setStatus(error?.message || ui("任务启动失败"), true);
    finishKnowledgeCardPlaceholder(task, "error", error?.message || ui("知识卡生成失败，请重试"));
    updateSelectionUi();
    updateCodexChatControls();
    finishTask({ status: "error", reason: error?.message || ui("任务启动失败") });
    return completion;
  }
}
...
function parseVisualSummary(answer) {
  ...
  try {
    ...
    if (start < 0 || end <= start) throw new Error(ui("没有 JSON"));
    const parsed = JSON.parse(fallback.slice(start, end + 1));
    return {
      title: String(parsed.title || ui("内容总结")).trim().slice(0, 36),
      ...
      relations: (Array.isArray(parsed.relations) ? parsed.relations : [])
        .map(relation => ({
          ...
          label: String(relation?.label || ui("关联")).trim().slice(0, 24)
        }))
    ...
    return {
      title: ui("内容总结"),
      summary: fallback.slice(0, 300) || ui("{0} 没有返回内容", aiRuntimeLabel()),
      highlights: [],
      relations: [],
      imageNotes: [],
      theme: "ocean"
...
async function renderVisualSummary(task, answer) {
  ...
  if (imageCount) {
    ...
    drawCenteredCanvasText(context, ui("视觉素材"), padding, y, 170, 48, 30, 1);
    y += 72;
    const gap = 24;
    ...
    images.forEach((image, index) => {
      ...
      const note = plan.imageNotes[index] || task.materials.filter(item => item.type === "image")[index]?.alt || ui("素材 {0}", index + 1);
      context.fillStyle = palette.ink;
  ...
  context.fillStyle = palette.paperRaised;
  context.font = `700 24px ${fontFamily}`;
  drawCenteredCanvasText(context, ui("内容结构"), padding, y, 170, 48, 30, 1);
  ...
  if (relations.length) {
    ...
    context.fillStyle = palette.paperRaised;
    context.font = `700 24px ${fontFamily}`;
    drawCenteredCanvasText(context, ui("关键关系"), padding, y, 170, 48, 30, 1);
    y += 72;
    ...
    relations.forEach((relation, index) => {
      ...
      const labelWidth = Math.min(190, Math.max(112, context.measureText(relation.label || ui("关联")).width + 42));
      ...
      context.fillStyle = palette.ink;
      context.font = `700 18px ${fontFamily}`;
      drawCenteredCanvasText(context, relation.label || ui("关联"), labelX + 8, top + 12, labelWidth - 16, 34, 22, 1);
    });
